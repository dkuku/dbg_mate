      ...
      Enum.flat_map(clauses, fn
        ...
        {:<-, _meta, [left, right]} ->
          ...
          quote do
            [
              value = unquote(right),
              unquote(acc_var) = [{unquote(Macro.escape(right)), value} | unquote(acc_var)],
  ...
  defp dbg_ast_to_debuggable_function(args, ast, _env) do
    arguments =
      Enum.map(args, fn arg ->
        quote do
          {unquote(Macro.escape(arg)), unquote(arg)}
        end
      end)

    quote do
      {:function, unquote(Macro.escape(ast)), unquote(arguments), unquote(ast)}
    end
  end
